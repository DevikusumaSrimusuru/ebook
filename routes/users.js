    ...
    try {
      ...
      try {
        ...
        const msg = `
          ...
          <h4 style="font-style: italic;">You are now a Customer of our <strong>GITAM Book Store</strong></h4>
          <p>You can now login and shopping with us!!</p>
          <h5>Thank you</h5>
          <h6>Admin, GITAM Book Store</h6>
          <img src="https://learn.gitam.edu/theme/image.php/gitam/theme/1710744240/favicon" alt="" style="height: 3em; width: 3em; border-radius: 50%">
        `;
        const mail = {
          from: `GITAM Book Store <${process.env.EMAIL_SENDER_ADDRESS}>`,
          to: savedUser.email,
          subject: 'Register on GITAM Book Store',
          text: 'Welcome to GITAM Book Store',
          html: msg
        };
    ...
    .exec(async (err, user) => {
      if (err) {
        ...
        const charge = await stripe.charges.create({
          ...
          description: 'GITAM Book Store Orders',
          amount: total * 100,
          currency: 'inr'
        });
        try {
          ...
          msg += `
				...
				<p style="color: orange">admin, GITAM Book Store</p>
				</body>
			`;
          const mail = {
            from: `GITAM Book Store <${process.env.EMAIL_SENDER_ADDRESS}>`,
            to: req.user.email,
            subject: 'Order Summary GITAM Book Store',
            text: 'Order Summary',
            html: msg
          };
          const info = await NodemailerTransporter.sendMail(mail);
      ...
      if (err) {
        ...
        try {
          ...
          msg += `
				...
				<h3 style="font-size: 1.2em;color: #f511ed">Your Order Id: ${order._id}</h3>
				<h2  style="color: red">Total Amount: ₹ ${total}</h2>
				<p  style="font-size: 1.2em; color: #d534eb">Your Order reciept Download from <a href="/">here</a></p>
				<h2 style="color: purple">Your Orders will be delivered to you within 3/4 business days</h3>
				<h4>Thanks For Shopping With Us</h4>
				<p style="color: orange">admin, GITAM Book Store</p>
				</body>
